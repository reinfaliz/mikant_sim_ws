...
namespace sail_luffing_controller_plugin
{
  ...
  
  {
	...
	void SailLuffingControllerPlugin::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
    	...
    	std::bind(&SailLuffingControllerPlugin::callbacksub1, this, std::placeholders::_1));
      	this->sub_2 = this->node_->create_subscription<std_msgs::msg::Float64MultiArray>(topic_name2, 10,
    	std::bind(&SailLuffingControllerPlugin::callbacksub2, this, std::placeholders::_1));
  	...
  	void SailLuffingControllerPlugin::callbacksub1(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
  	{
    		this->true_wind_angle = msg->data[0];
    		this->true_wind_speed = msg->data[1];
  	}
  	
  	void SailLuffingControllerPlugin::callbacksub2(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
  	{
    		this->x = msg->data[0];
    		this->y = msg->data[1];
    		this->phi = msg->data[2];
